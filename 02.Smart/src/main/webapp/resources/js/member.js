 ...
 */
 ...
 var member = {
	...
	common : {
		...
		max: 	{is:false, desc:"10자 미만 입력하세요"},
		space: 	{is:false, desc:"공백없이 입력하세요"},
	},
	
	userpw:{
		valid: {is:true, desc:"사용 가능합니다"},
	...
	showStatus:function(tag ){
		...
		tag.closest(".input-check").find(".desc").text(tag.attr("title") + " " + status.desc).removeClass("text-success text-danger").
	...
	userpw_status: function(pw , input){
		if(input ){
			$("[name=user_pw_ck]").val("");
			$("[name=user_pw_ck]").closest(".input-check").find(".desc").removeClass("text-success text-danger").text("");
		}
